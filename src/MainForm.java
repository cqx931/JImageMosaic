...
import plugins.*;
import plugins.saveTypes.BMPSaveType;
...
public class MainForm extends javax.swing.JFrame {
	...
	public MainForm() {
		JPopupMenu.setDefaultLightWeightPopupEnabled(false); // This is so the menu
																													// appears over the
																													// images in tabs.
		initComponents(); // Initialize GUI components.
		timer1.stop(); // Make sure the timer isn't ticking away.
		JFrame imageFrame = new JFrame(); // Create the frames for the Tabbed panel.
	...
	private void initComponents() {// GEN-BEGIN:initComponents
		...
		maintainLibButton.addActionListener(new java.awt.event.ActionListener() {
			public void actionPerformed(java.awt.event.ActionEvent evt) {
		...
		timer1.setDelay(5L);
		timer1.addTimerListener(new extras.TimerListener() {
			public void onTime(java.awt.event.ActionEvent evt) {
				timer1OnTime(evt);
			}
		});

		getContentPane().setLayout(new extras.AbsoluteLayout());
		...
		getContentPane().add(baseImageButton, new extras.AbsoluteConstraints(10, 500, 160, -1));
		...
		getContentPane().add(infoPane, new extras.AbsoluteConstraints(360, 40, 350, 200));

		infoTitleLabel.setText("Mosaic Info");
		getContentPane().add(infoTitleLabel, new extras.AbsoluteConstraints(360, 10, 220, 20));

		jLabel5.setText("Horizontal Tiles:");
		getContentPane().add(jLabel5, new extras.AbsoluteConstraints(370, 270, -1, -1));

		jLabel6.setText("Vertical Tiles:");
		getContentPane().add(jLabel6, new extras.AbsoluteConstraints(550, 270, -1, -1));
		...
		getContentPane().add(hTilesBox, new extras.AbsoluteConstraints(480, 270, 50, -1));
		...
		getContentPane().add(vTilesBox, new extras.AbsoluteConstraints(660, 270, 50, -1));
		...
		getContentPane().add(loadLibButton, new extras.AbsoluteConstraints(180, 500, 160, -1));

		jLabel1.setText("Tile Width (px): ");
		getContentPane().add(jLabel1, new extras.AbsoluteConstraints(370, 300, -1, -1));

		getContentPane().add(jTabbedPane2, new extras.AbsoluteConstraints(10, 10, 330, 480));
		...
		getContentPane().add(tileWidthBox, new extras.AbsoluteConstraints(480, 300, 50, -1));

		jLabel2.setText("Tile Height (px):");
		getContentPane().add(jLabel2, new extras.AbsoluteConstraints(550, 300, -1, -1));
		...
		getContentPane().add(tileHeightBox, new extras.AbsoluteConstraints(659, 300, 50, -1));

		jCheckBox1.setText("Maintain Aspect Ratio");
		jCheckBox1.setEnabled(false);
		getContentPane().add(jCheckBox1, new extras.AbsoluteConstraints(370, 330, -1, -1));
		...
		getContentPane().add(generateMapButton, new extras.AbsoluteConstraints(550, 330, 160, -1));

		getContentPane().add(formProgressBar, new extras.AbsoluteConstraints(350, 504, 360, 20));
		...
		getContentPane().add(renderFormButton, new extras.AbsoluteConstraints(560, 480, 80, 20));
		...
		getContentPane().add(stopButton, new extras.AbsoluteConstraints(640, 480, 70, 20));

		renderOptionsButton.setText("Render Options");
		renderOptionsButton.addActionListener(new java.awt.event.ActionListener() {
			public void actionPerformed(java.awt.event.ActionEvent evt) {
				renderOptionsButtonActionPerformed(evt);
			}
		});

		getContentPane().add(renderOptionsButton, new extras.AbsoluteConstraints(560, 460, 150, 20));

		renderSelectBox.setMaximumRowCount(5);
		renderSelectBox.setRenderer(new myComboBoxRenderer());
		getContentPane().add(renderSelectBox, new extras.AbsoluteConstraints(352, 480, 200, 20));

		jLabel3.setText("Rendering Algorithm");
		getContentPane().add(jLabel3, new extras.AbsoluteConstraints(350, 460, 200, -1));
	...
	private void renderOptionsButtonActionPerformed(java.awt.event.ActionEvent evt) {// GEN-FIRST:event_renderOptionsButtonActionPerformed
		optionBox.show();
	...
	// progressAction holds a string that tells what the task is.
		...
		if (progressAction == "render") {
			// Get the current location in the task, and its output, and update the
			// progress bar accordingly.
			formProgressBar.setValue(renderTask.getCurr());
			formProgressBar.setString(renderTask.getMessage() + "  [" + (int) (formProgressBar.getPercentComplete() * 100)
					+ "%]");
			if (renderTask.done()) { // If we are at the end of the task...
				formProgressBar.setString("Rendering Done!  [100%]"); // Show it.
				// Put the final image into a new scrollable tab in the tabbed panel.
				jTabbedPane2.add("Final", new ScrollingImagePanel(renderTask.getNewImage(),
						renderTask.getNewImage().getWidth(), renderTask.getNewImage().getHeight()));
				infoBox ib = new infoBox(MainForm.this, true, "Done Rendering!", "Done!"); // Display
																																										// a
																																										// success
																																										// message.
				ib.show();
				saveImageButton.setEnabled(true); // Allow the image to be saved.
				renderButton.setEnabled(true); // Enable/Disable appropriate controls.
				renderFormButton.setEnabled(true);
				stopButton.setEnabled(false);
				generateMapButton.setEnabled(true);
				hTilesBox.setEnabled(true);
		...
		} else if (progressAction == "fileload") { // Put file loading task stuff
																								// here.
		} else if (progressAction == "filesave") { // Put file saving task stuff
																								// here.
		} else if (progressAction == "mapCreate") { // If we are generating a map...
			// Get the current location in the task, and its output, and update the
			// progress bar accordingly.
			formProgressBar.setValue(mapTask.getCurr());
			formProgressBar.setString(mapTask.getMessage() + "  [" + (int) (formProgressBar.getPercentComplete() * 100)
					+ "%]");
			if (mapTask.done()) { // If we are at the end of the task...
				...
				if ((mapImageTab == null) && (mapTask.getOutImage() != null)) { // If
																																				...
																																				// display
																																				// one.
					mapImageTab = new ScrollingImagePanel(mapTask.getOutImage(), mapTask.getOutImage().getWidth(), mapTask
							.getOutImage().getHeight());
					jTabbedPane2.add("Map", mapImageTab);
				} else if (mapTask.getOutImage() != null) { // Otherwise, replace the
																										// old map tab with the new
																										// one.
					mapImageTab = new ScrollingImagePanel(mapTask.getOutImage(), mapTask.getOutImage().getWidth(), mapTask
							.getOutImage().getHeight());
					jTabbedPane2.setComponentAt(MAP_TAB_LOC, mapImageTab);
				}
				mapArray = mapTask.getMapArray(); // Hold on to the array of colors that
																					// was generated.
				generateMapButton.setEnabled(true);
				if ((images.getSize() > 0) && (mapImageTab != null)) {
					renderButton.setEnabled(true);
	...
	private void saveImageButtonActionPerformed(java.awt.event.ActionEvent evt) {// GEN-FIRST:event_saveImageButtonActionPerformed
		...
		if (returnVal == 0) { // If they actually selected a file,
			String saveFile = fileChoose.getSelectedFile().getPath(); // get the
																																// filename/path.
			BMPSaveType bst = new BMPSaveType(renderTask.getNewImage(), saveFile);
			if (bst.saveImageToFile() == false) { // Save the image to a file.
				...
				infoBox ib = new infoBox(MainForm.this, true, "Image Saved as " + fileChoose.getSelectedFile().toString(),
						"Saved");
				ib.show();
			}
		}
	...
	private void doRenderAction() {
		...
		progressAction = "render"; // Set the progressAction (for the timer/progress
																// indicator.)
		timer1.stop(); // Make sure the timer isn't running from other tasks.
		// Set up a new render task. In the future, this will be user-selectable.
		renderTask = new radiusRenderRandom(mapArray, images, baseImage, hTiles, vTiles, tileWidth, tileHeight,
				optionBox.getColorCorrectPercent(), optionBox.getTileRep(), optionBox.getMinDistNum(), optionBox.getTileOrder());
	...
	private void generateMapButtonActionPerformed(java.awt.event.ActionEvent evt) {// GEN-FIRST:event_generateMapButtonActionPerformed
		...
		mapTask = new defaultMapCreate(baseImage, hTiles, vTiles); // Create a new
																																// map task.
																																// This WILL be
																																// user-selectable.
	...
	// This is to calculate the data displayed in the Mosaic Info box, and display
	// it.
	private void showCalculatedInfo() {
		...
		// Show the data.
		finalDimText.setText("  Final Image Dimensions: " + finalWidth + " x " + finalHeight);
	}
	...
	private void baseImageButtonActionPerformed(java.awt.event.ActionEvent evt) {// GEN-FIRST:event_baseImageButtonActionPerformed
		progressAction = "fileload"; // set the progressAction for the
																	// timer/progress indicator (not currently
																	// implemented.)
		final JFileChooser fileChoose = new JFileChooser(); // Set up and show a
																												// file chooser dialog,
		fileChoose.setFileFilter(new myImageFilter()); // Filtering out things that
																										// aren't valid images.
		int returnVal = fileChoose.showOpenDialog(MainForm.this);
		if (returnVal == 0) { // If the user actually selected a file....

			this.baseImage = imageLoader.getRenderedImage(fileChoose.getSelectedFile().getPath()); // Load
																																															// the
																																															// image.
			if (baseImageTab == null) { // If there is no previously loaded base
																	// image, add this image to the tabbed panel.
				baseImageTab = new ScrollingImagePanel(baseImage, baseImage.getWidth(), baseImage.getHeight());
				jTabbedPane2.addTab("Base Image", baseImageTab);
				generateMapButton.setEnabled(true); // Allow the user to generate a map
																						// now.
			} else { // If there already is a base image loaded, replace the current
								// one...
				baseImageTab = new ScrollingImagePanel(baseImage, baseImage.getWidth(), baseImage.getHeight());
			...
			imgNameText = imgNameText.substring(imgNameText.lastIndexOf(Utils.getSlashType()) + 1, imgNameText.length());
			baseImageLabel.setText("  Base Image Name: " + imgNameText);
			baseImageDims.setText("  Base Image Dimensions: " + baseImage.getWidth() + " x " + baseImage.getHeight());
	...
	private void loadLibButtonActionPerformed(java.awt.event.ActionEvent evt) {// GEN-FIRST:event_loadLibButtonActionPerformed
		final JFileChooser fileChoose = new JFileChooser(); // Set up a file chooser
																												// dialog,
		fileChoose.setFileFilter(new myLibraryFilter()); // Showing only JImage
																											// Mosaic library files
																											// (*.jml).
		int returnVal = fileChoose.showOpenDialog(MainForm.this);
		if (returnVal == 0) { // If they chose a library...
			try {
				// Open the library.
				FileInputStream fis = new FileInputStream(fileChoose.getSelectedFile().getPath());
				...
				if (mapImageTab != null) { // If they already made the map, let them
																		// render.
					renderButton.setEnabled(true);
					renderFormButton.setEnabled(true);
				}
	...
	private RenderedImage baseImage; // The base image to use, as an image, not a
																		// filename.
	private Color[][] mapArray = new Color[500][500]; // The array for the color
																										// map.
	private TiledImage newImage; // The final generated image.
	private ScrollingImagePanel baseImageTab, mapImageTab; // Scrolling panels for
																													// the image and map
																													// tabs to go into.
	private String progressAction = "render"; // Tells timer/progress indicator
																						// what task is being performed.
	// These two will be replaced when a classloader is implemented, and the user
