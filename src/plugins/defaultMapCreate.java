...
public class defaultMapCreate extends mapCreatePlugin {
	...
	private Color[][] mapArray = new Color[500][500]; // The array of colors,
	...
	// Constructor.
	public defaultMapCreate(RenderedImage ri, int h, int v) { // the base image,
																														// hTiles and
																														// vTiles.
		super.setTaskLen(h * v); // The length of the task is the number of tiles
															// (hTiles * vTiles).
	...
	class createMap { // overwrite this class to include your algorithm.
		...
		createMap() {
			...
			// Calculate how tall and wide the chunks we are finding the average color
			// for are.
			// A "pixel" is a large average-color representation of an area on the
			// base image.
			int pixelWidth = (int) (baseImage.getWidth() / hTiles);
			int pixelHeight = (int) (baseImage.getHeight() / vTiles);

			// Create a new image, used as the output. Base it on the same color model
			...
			for (int smallY = 0; smallY < vTiles; smallY++) {
				for (int smallX = 0; smallX < hTiles; smallX++) {
					...
					try {
						...
						// Create a region of interest on the source image that starts at
						// the starting pixels
						// we just calculated, and extends the height and width of the
						// block.
						...
						double[] mean = (double[]) meanImage.getProperty("mean"); // Extract
																																			// the
																																			// data as
																																			// doubles.
